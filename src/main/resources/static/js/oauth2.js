...
// const ACCESS_TOKEN_REDIRECT_URI = "https://localhost:8080/redirect"; // куда auth server будет отправлять access token и другие токены
const RESOURCE_SERVER_URL = "https://localhost:8901";
...
const CLIENT_ROOT_URI = "https://localhost:8080"
let accessToken = ""
let refreshToken = ""
let idToken = ""
...
function checkAuthCode() {
    ...
    const errorDescription = url.get("error_description");
...
function initAccessToken() {
    ...
    console.log("init codeVerifier = " + codeVerifier);
...
function generateCodeVerifier() {
    const randomByteArray = new Uint8Array(43);
...
async function generateCodeChallenge(codeVerifier) {
    ...
    const encodedValue = textEncoder.encode(codeVerifier); // кодируем в массив байтов ранее полученный code_verifier
    const digest = await window.crypto.subtle.digest(SHA_256, encodedValue);
...
function requestAuthCode(state, codeChallenge) {
    ...
    authUrl += "&redirect_uri=" + CLIENT_ROOT_URI; // куда auth server будет отправлять ответ
...
function requestTokens(stateFromAuthServer, authCode) { // idea может показывать, что функция нидге не используется, но это не так, просто он не может определить вызов из другого window
    ...
    // const originalState = document.getElementById("originalState").innerHTML;
    ...
    if (stateFromAuthServer === originalState) {
        ...
        // const codeVerifier = document.getElementById("codeVerifier").innerHTML;
        const codeVerifier = localStorage.getItem(CODE_VERIFIER_KEY)
        console.log("request get token codeVerifier = " + codeVerifier);
        ...
        const data = {
            ...
            "redirect_uri": CLIENT_ROOT_URI // куда auth server будет отправлять ответ
        };
...
function accessTokenResponse(data, status, jqXHR) { // // эти параметры передаются автоматически, data будет в формате JSON
    ...
    const idTokenJSON = getJsonPayload(idToken)
    console.log(idTokenJSON)
    document.getElementById("name").innerHTML = "Hello, "  + idTokenJSON["name"]
...
function getDataFromResourceServer() {
    $.ajax({ // ajax запрос для параллельного вызова
        ...
        type: "GET", // тип запроса обязательно должен быть POST
        url: RESOURCE_SERVER_URL + "/user/data", // адрес обращения
        success: resourceServerResponse, // (callback) какой метод вызывать после выполнения запроса (туда будет передан результат)
        error: resourceServerError,
        dataType: "text" // в каком формате получем ответ от auth server
    });
}
...
function logout() {
    ...
    authUrl += "?post_logout_redirect_uri=" + CLIENT_ROOT_URI;
